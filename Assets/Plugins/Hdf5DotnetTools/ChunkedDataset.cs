...
namespace Hdf5DotNetTools {
	public class ChunkedDataset<T> : IDisposable where T : struct {
		...
		public ChunkedDataset(string name, long groupId, T[,] dataset) : this(name, groupId, new ulong[] { Convert.ToUInt64(dataset.GetLongLength(0)), Convert.ToUInt64(dataset.GetLongLength(1)) }) {
		...
		private ulong[] GetDims(Array dset) {
			return Enumerable.Range(0, dset.Rank).Select(i => { return (ulong)dset.GetLength(i); }).ToArray();
		}
