...
namespace Hdf5DotNetTools {
	...
	public static partial class Hdf5 {
		...
		public static Array ReadAttributes<T>(long groupId, string name) {
		...
		public static IEnumerable<string> ReadStringAttributes(long groupId, string name) {
		...
		public static Array ReadPrimitiveAttributes<T>(long groupId, string name) //where T : struct
		{
			Type type = typeof(T);
		...
		public static int WriteStringAttribute(long groupId, string name, string str, string datasetName = null) {
			return WriteStringAttributes(groupId, name, new string[] { str }, datasetName);
		}

		public static int WriteStringAttributes(long groupId, string name, IEnumerable<string> strs, string datasetName = null) {
			long tmpId = groupId;
			if (!string.IsNullOrWhiteSpace(datasetName)) {
				long datasetId = H5D.open(groupId, datasetName);
				if (datasetId > 0)
					groupId = datasetId;
			}
			...
			long spaceId = H5S.create_simple(1,
		...
		public static void WriteAttributes<T>(long groupId, string name, Array attributes, string datasetName = null) //
		{
		...
		public static int WritePrimitiveAttribute<T>(long groupId, string name, Array attributes, string datasetName = null) //where T : struct
		{
			var tmpId = groupId;
			if (!string.IsNullOrWhiteSpace(datasetName)) {
			...
			ulong[] dims = Enumerable.Range(0, rank).Select(i => { return (ulong)attributes.GetLength(i); }).ToArray();
			ulong[] maxDims = null;
